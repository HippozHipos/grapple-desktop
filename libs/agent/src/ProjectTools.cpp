...
constexpr const char CameraSetTransformKeyframeSchema[] = R"json({
  "type": "object",
  "additionalProperties": false,
  "required": ["cameraNodeId", "paramName", "time", "value"],
  "properties": {
    "cameraNodeId": {"type": "string", "minLength": 1},
    "paramName": {"enum": ["position_x", "position_y", "zoom"]},
    "time": {"type": "number"},
    "value": {"type": "number"}
  }
})json";
...
struct CameraTransformEffectRef {
  const graph::GraphNode* node = nullptr;
  const timeline::EffectPayload* payload = nullptr;
};

foundation::Result<std::optional<CameraTransformEffectRef>> findCameraTransformEffect(
  const project::ProjectSnapshot& snapshot,
  const foundation::NodeId& cameraId
) {
  for (const graph::GraphEdge& edge : snapshot.graph.edges()) {
    ...
    if (effectPayload->implementation.kind == timeline::EffectImplementationKind::Builtin &&
      ...
      return std::optional<CameraTransformEffectRef>{CameraTransformEffectRef{effectNode, effectPayload}};
    }
  }

  return std::optional<CameraTransformEffectRef>{};
}

const timeline::Param* findEffectParam(const timeline::EffectPayload& payload, const std::string& paramName) {
...
foundation::Result<void> registerProjectTools(AgentToolRegistry& registry) {
  ...
  registered = registry.registerTool(makeCameraSetTransformKeyframeTool());
  if (!registered) {
    return registered.error();
  }
...
AgentTool makeCameraAddTransformControlsTool() {
  return AgentTool{
    ...
    [](const ToolCall& call, AgentToolContext& context) -> foundation::Result<ToolResult> {
      ...
      auto existingTransform = findCameraTransformEffect(snapshot.value(), cameraId);
      if (!existingTransform) {
        return existingTransform.error();
      }
      if (existingTransform.value().has_value()) {
...
AgentTool makeCameraSetTransformKeyframeTool() {
  return AgentTool{
    foundation::ToolId{"tool_camera_set_transform_keyframe"},
    "camera.set_transform_keyframe",
    "Set Camera Transform Keyframe",
    "Sets or updates one Camera Transform parameter keyframe on an existing camera. The tool finds the camera's editable transform effect and owns keyframe id allocation.",
    CameraSetTransformKeyframeSchema,
    [](const ToolCall& call, AgentToolContext& context) -> foundation::Result<ToolResult> {
      auto arguments = parseArguments(call.arguments);
      if (!arguments) {
        return arguments.error();
      }
      auto members = requireOnlyMembers(arguments.value(), {"cameraNodeId", "paramName", "time", "value"}, "$");
      if (!members) {
        return members.error();
      }
      auto cameraNodeId = requiredStringMember(arguments.value(), "cameraNodeId", "$");
      ...
      if (paramName.value() != effects::builtin_effect::PositionXParam &&
          paramName.value() != effects::builtin_effect::PositionYParam &&
          paramName.value() != effects::builtin_effect::ZoomParam) {
        return argumentError("$.paramName", "Expected a Camera Transform parameter name.");
      }
      auto time = requiredDoubleMember(arguments.value(), "time", "$");
      if (!time) {
        return time.error();
      }
      auto value = requiredDoubleMember(arguments.value(), "value", "$");
      if (!value) {
        return value.error();
      }

      auto snapshot = readProjectSnapshot(context, "camera.set_transform_keyframe");
      if (!snapshot) {
        return snapshot.error();
      }
      ...
      const graph::GraphNode* cameraNode = snapshot.value().graph.findNode(cameraId);
      if (cameraNode == nullptr || cameraNode->kind != graph::NodeKind::Camera) {
        return foundation::Error{"agent.camera_missing", "Camera transform keyframes require an existing camera node."};
      }

      auto transformEffect = findCameraTransformEffect(snapshot.value(), cameraId);
      if (!transformEffect) {
        return transformEffect.error();
      }
      if (!transformEffect.value().has_value()) {
        return foundation::Error{"agent.camera_transform_missing", "Camera transform keyframes require existing Camera Transform controls."};
      }

      const timeline::Param* param = findEffectParam(*transformEffect.value()->payload, paramName.value());
      if (param == nullptr || !std::holds_alternative<double>(param->value)) {
        return foundation::Error{"agent.camera_transform_param_missing", "Camera Transform keyframe parameter must exist and be numeric."};
      }

      const foundation::TimeSeconds keyframeTime{time.value()};
      std::optional<foundation::KeyframeId> existingKeyframeId;
      for (const timeline::Param::Keyframe& keyframe : param->keyframes) {
        if (keyframe.time == keyframeTime) {
          existingKeyframeId = keyframe.id;
          break;
        }
      }

      const foundation::CommandId commandId = context.ids.nextCommandId();
      const foundation::KeyframeId keyframeId = existingKeyframeId.has_value()
        ? existingKeyframeId.value()
        : context.ids.nextKeyframeId(paramName.value());
      auto command = context.commands.apply(project::ProjectCommandEnvelope{
        commandId,
        call.projectId,
        call.expectedRevision,
        ...
        project::UpsertEffectParamKeyframeCommand{
          transformEffect.value()->node->id,
          paramName.value(),
          timeline::Param::Keyframe{
            keyframeId,
            keyframeTime,
            timeline::ParamValue{value.value()}
          }
        }
      });
      if (!command) {
      ...
      payload << '{'
              << "\"commandId\":" << foundation::jsonQuoted(commandId.value())
              << ",\"cameraNodeId\":" << foundation::jsonQuoted(cameraId.value())
              << ",\"effectNodeId\":" << foundation::jsonQuoted(transformEffect.value()->node->id.value())
